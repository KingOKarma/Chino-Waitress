...
export const command: Command = {
    cooldown: 3,
    description: "Change the mentionability of the rotd role",
    example: ["!uptime"],
    group: "staff",
    staffOnly: true,
    guildOnly: true,
    ...
    run: async ({ client, msg }) => {
        ...
        try {
            // I dont have the effort rn to be asked to do all the db stuff, so heres a hardcode
            const role = await msg.guild?.roles.fetch("659832485344968725");
